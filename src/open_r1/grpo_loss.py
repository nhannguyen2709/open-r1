...
class LigerFusedLinearGRPOFunction(LigerFusedLinearRLHFBase):
    ...
    def rlhf_loss_fn(
        ...
        **kwargs,
        ...
        chosen_token_logprobs = log_probs.gather(
            dim=-1, index=chosen_tokens.unsqueeze(-1)
        ).squeeze(
            -1
        )  # (batch_size, seq_len)
        ...
        metrics = (
            ...
            (
                (kl_div * attention_mask).sum(dim=1) / attention_mask.sum(dim=1)
            ).mean(),  # mean KL div
        )
    ...
    def backward(ctx, grad_output, *grad_metrics):
        ...
        return (
            *grads[
                :5
            ],  # grad_input, grad_weight, grad_attention_mask, grad_rewards, grad_bias
            None,  # grad_ref_input
